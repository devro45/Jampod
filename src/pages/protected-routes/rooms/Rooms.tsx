...
import { IRoom, IDataResponse, IRoomCard } from '../../../types'
import styles from "../PR.module.css"
import { AxiosResponse } from 'axios'

const Rooms: React.FC = () => {
  const [showModal, setShowModal] = useState<boolean>(false);
  const [rooms, setRooms] = useState<IRoom[]>([]); // Initialize as empty array
  ...
  useEffect(() => {
    const fetchRoom = async ():Promise<void> => {
      try {
        const { data }: AxiosResponse<IDataResponse> = await getAllRooms();
        console.log(data);
  ...
  return (
    <>
      ...
      <div className='max-w-screen-xl mt-4 px-6 sm:px-8 lg:px-16 mx-auto grid grid-flow-row py-3 sm:py-4'>
        ...
        {rooms && rooms.length > 0 && ( // Check if rooms is not empty and has length greater than 0
